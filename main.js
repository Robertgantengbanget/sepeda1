...
function updateCharts(t, zs, zu, shock, pivot) {
  ...
  charts.pivot = createOrUpdateChart(charts.pivot, ctx1, {
    labels: t,
    datasets: [{ label: 'Wheel Travel (mm)', data: pivot, borderColor: '#667eea', tension: 0.1, borderWidth: 2 }]
  }, { ...commonOptions, scales: { ...commonOptions.scales, y: { title: { display: true, text: 'mm' } } } });

  charts.zs = createOrUpdateChart(charts.zs, ctx2, {
    labels: t,
    datasets: [{ label: 'Sprung Mass (m)', data: zs, borderColor: '#28a745', tension: 0.1, borderWidth: 2 }]
  }, { ...commonOptions, scales: { ...commonOptions.scales, y: { title: { display: true, text: 'm' } } } });

  charts.zu = createOrUpdateChart(charts.zu, ctx3, {
    labels: t,
    datasets: [{ label: 'Unsprung Mass (m)', data: zu, borderColor: '#dc3545', tension: 0.1, borderWidth: 2 }]
  }, { ...commonOptions, scales: { ...commonOptions.scales, y: { title: { display: true, text: 'm' } } } });

  charts.y = createOrUpdateChart(charts.y, ctx4, {
    labels: t,
    datasets: [{ label: 'Shock Travel (mm)', data: shock, borderColor: '#ffc107', tension: 0.1, borderWidth: 2 }]
  }, { ...commonOptions, scales: { ...commonOptions.scales, y: { title: { display: true, text: 'mm' } } } });
}
